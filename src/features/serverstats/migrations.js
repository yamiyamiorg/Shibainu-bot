...
async function migrate(dbPath) {
  ...
  // ── system flags（初回のみ処理のフラグ保存など） ───────────────────────
  await run(db, `CREATE TABLE IF NOT EXISTS system_flags (
  ...
  await run(db, `CREATE TABLE IF NOT EXISTS vc_excluded_channels (
